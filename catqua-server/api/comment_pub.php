...
if (strcmp(trim($pid),"")==0) $flag = false;
if (strcmp(trim($content),"")==0) $flag = false;
if ($flag) {
	$query_pid = sprintf("SELECT * FROM `post` WHERE id = %s", $pid);
	$res = mysql_query($query_pid, $dblink);
...
$query_rs = sprintf("INSERT INTO `comment` (uid, usrname, pid, content) 
VALUES (%s, \"%s\", %s, \"%s\")",$uid,$_COOKIE["username"],$pid,$content);
