...
$query_rs = sprintf("SELECT * FROM `user` WHERE user.name='%s' AND user.pwd='%s'",$username,$pwd);
...
if($row = mysql_fetch_assoc($rs)) {
  ...
  <notice>
	...
	echo "<catquq><result><errorCode>0</errorCode><errorMessage><![CDATA[用户名或密码错]]></errorMessage></result><notice><atmeCount>0</atmeCount><msgCount>0</msgCount><reviewCount>0</reviewCount><newFansCount>0</newFansCount></notice></catquq>";
}
