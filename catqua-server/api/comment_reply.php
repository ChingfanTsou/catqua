...
$query_rs = sprintf("UPDATE `post` SET answerCount = answerCount+1 WHERE id=%s",$row['pid']);
$rs = mysql_query($query_rs, $dblink);
$query_rs = sprintf("UPDATE `post` SET pubDate = CURRENT_TIMESTAMP WHERE id=%s",$row['pid']);
$rs = mysql_query($query_rs, $dblink);
